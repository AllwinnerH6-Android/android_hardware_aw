...
#undef NDEBUG
#define LOG_TAG "CameraHALv3_4-METADATACOMMON"
...
static int UpdateMetadata(CameraMetadata* metadata,
  ...
  if (res) {
    HAL_LOGE("Failed to update metadata tag 0x%x", tag);
    return -ENODEV;
  }
  return 0;
}
...
static void GetDataPointer(camera_metadata_entry_t entry,
                           const uint8_t** val) {
  *val = entry.data.u8;
}
...
static int EraseTag(CameraMetadata* metadata,
                          int32_t tag) {

  int res = metadata->erase(tag);
  if (res) {
      HAL_LOGE("Erase tag 0x%x fail.", tag);\
  }
...
static int SingleTagValue(const CameraMetadata& metadata,
  ...
  if (entry.count == 0) {
    HAL_LOGE("Metadata tag 0x%x is empty.", tag);
    return -ENOENT;
...
// Singleton.
template <typename T>
static int SingleTagValue(CameraMetadata* metadata,
                          int32_t tag,
                          T* val) {
  if (!val) {
    HAL_LOGE("Null pointer passed to SingleTagValue.");
...
static int SingleTagValue(const CameraMetadata& metadata,
  ...
  if (entry.count == 0) {
    ...
    HAL_LOGW(
        ...
        "exactly %d values (had %d), set count:%d to %d.",
        tag,
        N,
        entry.count,
        N,
