...
#define OUTPUT (1024) // must be 16x, 640x for webrtc
...
#define AC100_XML_PATH "/vendor/etc/ac100_paths.xml"
...
void enable_hub_codec(struct mixer *mixer);
void enable_hub_hdmi(struct mixer *mixer);
void enable_hub_bt(struct mixer *mixer);
void enable_hub_codec_in(struct mixer *mixer);
void enable_hub_bt_in(struct mixer *mixer);
void enable_daudio0_in(struct mixer *mixer);
...
void close_daudio0_in(struct mixer *mixer);
...
static char * get_audio_devices(struct sunxi_audio_device *adev, int in_out)
{
    ...
    for(card = 0; card < MAX_AUDIO_DEVICES; card++)
    {
        if (adev->dev_manager[card].flag_exist == true
                ...
                && (strcmp(adev->dev_manager[card].name, "AUDIO_HDMI_IN") != 0))
        {
            if (adev->dev_manager[card].flag_in == AUDIO_IN)
            {
...
static int start_output_stream(struct sunxi_stream_out *out)
{
    ...
    for (index = 0; index < MAX_AUDIO_DEVICES; index++)
    {
            ...
            && adev->dev_manager[index].flag_out_active)
        {
            ...
            if(card == adev->cardCODEC){
                ...
                init_ac100_input_output_path(adev,OUT_DEVICE_SPEAKER_AND_HEADSET,IN_SOURCE_MAINMIC);
            }
#endif
#ifdef KARAOK_AUDIO_DEVICE
            card = 0;
    ...
    enable_hub_codec(adev->audio_hub_mixer);
    enable_hub_hdmi(adev->audio_hub_mixer);
    enable_hub_bt(adev->audio_hub_mixer);
    return 0;
}
...
//	if ((adev->mode == AUDIO_MODE_IN_CALL&&adev->out_device==AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET)||adev->mode == AUDIO_MODE_MODE_FACTORY_TEST || adev->mode == AUDIO_MODE_FM)
    ...
    debug_dump_data(buffer, out_frames * frame_size, &out->dd_write_out);
...
static int start_input_stream(struct sunxi_stream_in *in)
{
...
#ifdef USE_HDMI_IN
    if(adev->audio_source == AUDIO_SOURCE_HDMI_IN)
    {
        card = adev->cardHDMIIN;
        in_ajust_rate = SAMPLING_RATE_44K;
    }
#endif
    ALOGD("audio_devide_in card = %d",card);
    if(card == adev->cardHDMIIN)
    {
        adev->in_hdmi_pcm = pcm_open_req(card, 0, PCM_IN, &in->config, in_ajust_rate);
        if (!pcm_is_ready(adev->in_hdmi_pcm)) {
            ALOGE("cannot open pcm_in driver: %s", pcm_get_error(adev->in_hdmi_pcm));
            pcm_close(adev->in_hdmi_pcm);
        ...
        pcm_prepare(adev->in_hdmi_pcm);
        pcm_prepare(in->pcm);
        close_hub_bt_in(adev->audio_hub_mixer);
        close_hub_codec_in(adev->audio_hub_mixer);
		enable_daudio0_in(adev->audio_hub_mixer);
    }
    else if(card == adev->cardCODEC)
    {
        ...
        pcm_prepare(in->pcm);
        close_hub_bt_in(adev->audio_hub_mixer);
        close_daudio0_in(adev->audio_hub_mixer);
        enable_hub_codec_in(adev->audio_hub_mixer);
    }
    else if(card == adev->cardBT)
    {
        ...
        close_daudio0_in(adev->audio_hub_mixer);
        close_hub_codec_in(adev->audio_hub_mixer);
        enable_hub_bt_in(adev->audio_hub_mixer);
    }
    else
    {
        close_hub_bt_in(adev->audio_hub_mixer);
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        val = atoi(value);
#if USE_HDMI_IN
        //AUDIO_SOURCE_MIC ->CODEC
        //AUDIO_SOURCE_HDMI_IN ->HDMI_IN
        adev->audio_source = val;
#endif
                                   ...
                                   int64_t *frames, int64_t *time)
{
    ...
    ALOGV("in_get_capture_position! in->pcm :%d", in->pcm);
    pthread_mutex_lock(&in->lock);
    if (in->pcm) {
        ALOGV("in_get_capture_position! %d", __LINE__);
        struct timespec timestamp;
        unsigned int avail;
        if (pcm_get_htimestamp(in->pcm, &avail, &timestamp) == 0) {
            *frames = in->frames_read + avail;
            *time = timestamp.tv_sec * 1000000000LL + timestamp.tv_nsec;
            ALOGV("in_get_capture_position:frames is %lld, time is %lld", *frames, *time);
            ret = 0;
        }
    }
    pthread_mutex_unlock(&in->lock);
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    if (ret >= 0)
    {
        ALOGD("adev PROP_RAWDATA_KEY: %s", value);
        pthread_mutex_lock(&adev->lock);
    ...
    if (ret >= 0)
    {
        ALOGD("adev AUDIO_PARAMETER_DEVICES_OUT_ACTIVE: %s", value);
        pthread_mutex_lock(&adev->lock);
        if (adev->raw_flag == true)
        {
            ALOGW("in raw mode, should not set other audio out devices");
                     ...
                     hw_device_t** device)
{
    ...
    getCardNumbyName(adev, AUDIO_NAME_HDMI_IN, &adev->cardHDMIIN);
