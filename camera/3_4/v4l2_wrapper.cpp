...
V4L2Wrapper* V4L2Wrapper::NewV4L2Wrapper(const int id) {
  HAL_LOG_ENTER();
  #if 0
...
int V4L2Wrapper::Connect(STREAM_SERIAL ss) {
  ...
  std::shared_ptr<V4L2Stream> stream(V4L2Stream::NewV4L2Stream(device_id_, device_path));
  if (!stream) {
    HAL_LOGE("Failed to initialize stream helper.");
    return -1;
  }
