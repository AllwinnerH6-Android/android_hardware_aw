...
V4L2Stream* V4L2Stream::NewV4L2Stream(const int id, const std::string device_path) {
  return new V4L2Stream(id, device_path);
}
...
int V4L2Stream::Connect() {
  ...
  while(try_num--) {
    ...
    fd = TEMP_FAILURE_RETRY(open(device_path_.c_str(), O_RDWR | O_NONBLOCK));
    if (fd < 0) {
...
void V4L2Stream::Disconnect() {
  ...
  int res = TEMP_FAILURE_RETRY(close(device_fd_.get()));
  HAL_LOGV("Close device path:%s, fd:%d, res: %s", device_path_.c_str(), device_fd_.get(), strerror(res));
  if (res) {
    HAL_LOGW("Disconnected from camera device %s. fd:%d encount err(%s).",device_path_.c_str(), device_fd_.get(), strerror(res));
  }
...
int V4L2Stream::IoctlLocked(int request, T data) {
  ...
  HAL_LOGV("Stream fd:%d..", device_fd_.get());
...
int V4L2Stream::StreamOff() {
  ...
  HAL_LOGD("After stream %d, ind:%d turned off.", device_id_, device_fd_.get());
#ifdef USE_ISP
  mAWIspApi->awIspStop(mIspId);
  HAL_LOGV("Stream %d, ind:%d awIspStop.", device_id_, device_fd_.get());
#endif
  ...
  has_StreamOn = false;
  HAL_LOGV("Stream %d, ind:%d turned off.", device_id_, device_fd_.get());
  return 0;
}
...
int V4L2Stream::flush() {
  ...
  HAL_LOGV("Stream %d, ss:%d, ind:%d flush.", device_id_, device_ss_, device_fd_.get());
  return 0;
}
...
int V4L2Stream::PrepareBuffer() {
  ...
  for (int i = 0; i < buffers_.size(); i++)
  {
    ...
    HAL_LOGD("index: %d, fd: %d, mem: %lx, len: %d, offset: %p", i, device_fd_.get(),(unsigned long)mMapMem.mem[i], device_buffer.m.planes[0].length, device_buffer.m.offset);
...
int V4L2Stream::DequeueBuffer(void ** src_addr_) {
  if (!format_) {
    HAL_LOGV(
...
int V4L2Stream::WaitCameraReady()
{
  ...
  FD_ZERO(&fds);
  FD_SET(device_fd_.get(), &fds);

  /* Timeout */
  tv.tv_sec  = 1;
  tv.tv_usec = 0;

  r = select(device_fd_.get() + 1, &fds, NULL, NULL, &tv);
  if (r == -1)
  {
