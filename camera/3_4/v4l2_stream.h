...
class V4L2Stream : public virtual android::RefBase  {
 ...
 public:
  ...
  static V4L2Stream* NewV4L2Stream(const int id, const std::string device_path);
  virtual ~V4L2Stream();
  ...
  virtual int DequeueBuffer(void ** src_addr_);
...
private:
  ...
  V4L2Stream(const int id, const std::string device_path);
