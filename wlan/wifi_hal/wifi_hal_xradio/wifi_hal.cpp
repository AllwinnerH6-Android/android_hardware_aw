...
static nl_sock * wifi_create_nl_socket(int port)
{
    ...
    if (nl_connect(sock, NETLINK_GENERIC)) {
        ALOGV("Could not connect handle");
        nl_socket_free(sock);
        return NULL;
    }
    ...
    if (nl_socket_set_nonblocking(sock)) {
        ALOGV("Could make socket non-blocking");
        nl_socket_free(sock);
        return NULL;
    }
    */
...
/*initialize function pointer table with xradio HAL API*/
...
wifi_error wifi_initialize(wifi_handle *handle)
{
    ...
    if (info == NULL) {
        ALOGV("Could not allocate hal_info");
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, info->cleanup_socks) == -1) {
        ALOGV("Could not create cleanup sockets");
        free(info);
        return WIFI_ERROR_UNKNOWN;
    }
    ...
    if (info->nl80211_family_id < 0) {
        ALOGV("Could not resolve nl80211 familty id");
        nl_socket_free(cmd_sock);
        nl_socket_free(event_sock);
        free(info);
        return WIFI_ERROR_UNKNOWN;
...
static int wifi_add_membership(wifi_handle handle, const char *group)
{
    ...
    if (id < 0) {
        ALOGV("Could not find group %s", group);
        return id;
    }

    int ret = nl_socket_add_membership(info->event_sock, id);
    if (ret < 0) {
        ALOGV("Could not add membership to group %s", group);
    }
...
void wifi_cleanup(wifi_handle handle, wifi_cleaned_up_handler handler)
{
    ...
    if (TEMP_FAILURE_RETRY(write(info->cleanup_socks[0], "Exit", 4)) < 1) {
        ...
        ALOGV("could not write to the cleanup socket");
    } else {
        ...
        ALOGV("%s: Read after POLL returned %zd, error no = %d (%s)", __FUNCTION__,
               result, errno, strerror(errno));
        if (strncmp(buf, "Done", 4) == 0) {
            ALOGV("Event processing terminated");
        } else {
            ALOGD("Rx'ed %s", buf);
        }
    }
...
void wifi_event_loop(wifi_handle handle)
{
    ...
    do {
        ...
        if (result < 0) {
            // ALOGV("Error polling socket");
        } else if (pfd[0].revents & POLLERR) {
            ALOGV("POLL Error; error no = %d (%s)", errno, strerror(errno));
            ssize_t result2 = TEMP_FAILURE_RETRY(read(pfd[0].fd, buf, sizeof(buf)));
            ALOGV("Read after POLL returned %zd, error no = %d (%s)", result2,
                  errno, strerror(errno));
        } else if (pfd[0].revents & POLLHUP) {
            ALOGV("Remote side hung up");
            break;
            ...
            ALOGV("Unknown event - %0x, %0x", pfd[0].revents, pfd[1].revents);
        }
    } while (!info->clean_up);
...
public:
    ...
    int start() {
        ...
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to create request; result = %d", result);
            return result;
        }

        result = requestResponse(request);
        if (result != WIFI_SUCCESS) {
            ALOGV("failed to set scanning mac OUI; result = %d", result);
        }
...
public:
    ...
    virtual int create() {
        ...
        if (ret < 0) {
            ALOGV("Can't create message to send to driver - %d", ret);
            return ret;
        }
   ...
   int createRequest(WifiRequest& request, int enable) {
    ...
    virtual int cancel() {
        ...
        if (result != WIFI_SUCCESS) {
            ...
            result = requestResponse(request);
            if (result != WIFI_SUCCESS) {
                ALOGV("failed to stop RSSI monitoring = %d", result);
            }
        }
        unregisterVendorHandler(GOOGLE_OUI, GOOGLE_RSSI_MONITOR_EVENT);
        return WIFI_SUCCESS;
...
class AndroidPktFilterCommand : public WifiCommand {
    ...
    int handleResponse(WifiEvent& reply) {
        ...
        if (vendor_data == NULL || len == 0) {
            ALOGV("no vendor data in SetAPFCommand response; ignoring it");
            return NL_SKIP;
        }
        if( mReqType == SET_APF_PROGRAM) {
            ...
            for (nl_iterator it(vendor_data); it.has_next(); it.next()) {
                if (it.get_type() == APF_ATTRIBUTE_VERSION) {
                    ...
                    *mMaxLen = it.get_u32();
                    ...
                    ALOGV("Ignoring invalid attribute type = %d, size = %d",
...
public:
    ...
    virtual int create() {
        ...
        if(feature_type == ANDR_WIFI_ATTRIBUTE_NUM_FEATURE_SET) {
            ...
            ALOGV("Unknown feature type %d", feature_type);
            return -1;
        }
...
protected:
    virtual int handleResponse(WifiEvent& reply) {
        ...
        if (vendor_data == NULL || len == 0) {
            ALOGV("no vendor data in GetFeatureSetCommand response; ignoring it");
            return NL_SKIP;
        }
        if(feature_type == ANDR_WIFI_ATTRIBUTE_NUM_FEATURE_SET) {
            void *data = reply.get_vendor_data();
            if(!fset) {
                ALOGV("Buffers pointers not set");
                return NL_SKIP;
            }
